...
{
   public static class DebitNoteProvider
    {
       ...
       public static long getMaxIdDebitNoteId(long financialYearID)
        {
            try
            {
                ...
                long salesRetrunInvoiceId = sqlH.ExecuteAsScalarLong("[dbo].[Usp_getMaxDebitNoteInvoiceID]", parameter);
        ...
        public static int addDebitNote(SalesReturnDetails creditNote, PusrchaseSaleAccount sale)
        {
            using (var Conn = new SqlConnection(_connectionString))
            {
                ...
                List<KeyValuePair<string, object>> parameter = new List<KeyValuePair<string, object>>();
                parameter.Add(new KeyValuePair<string, object>("@debitNoteId", creditNote.salesReturnId));
                parameter.Add(new KeyValuePair<string, object>("@debitNoteInvoiceId", creditNote.salesReturnInvoiceId));
                parameter.Add(new KeyValuePair<string, object>("@financialYearID", creditNote.financialYearID));
                parameter.Add(new KeyValuePair<string, object>("@debitNoteDate", creditNote.salesReturnDate));
                ...
                parameter.Add(new KeyValuePair<string, object>("@dealerId", creditNote.customerId));
                parameter.Add(new KeyValuePair<string, object>("@debitNoteType", creditNote.salesType));
                parameter.Add(new KeyValuePair<string, object>("@debitNoteAmount", creditNote.totalbillAmount));
                ...
                int i = sqlH.ExecuteNonQueryTM("[dbo].[Usp_addDebitNoteMaster]", parameter,Conn,trans);
                if (creditNote.isDelete == false)
                {
                    ...
                    int s = sqlH.ExecuteNonQueryTM("[dbo].[Usp_addAllDebitNoteAccount]", parameter2,Conn,trans);
                }
                int result = 0;
                for (int k = 0; k < creditNote.SaleRetrunItemTable.Rows.Count; k++)
                {
                    ...
                    parameter1.Add(new KeyValuePair<string, object>("@debitNoteId", creditNote.salesReturnId));
                    parameter1.Add(new KeyValuePair<string, object>("@debitNoteInvoiceId", creditNote.salesReturnInvoiceId));
                    parameter1.Add(new KeyValuePair<string, object>("@financialYearID", creditNote.financialYearID));
                    parameter1.Add(new KeyValuePair<string, object>("@debitNoteDetailsId", creditNote.SaleRetrunItemTable.Rows[k]["salesReturnItemDetailsId"]));//4
                    ...
                    SqlHandler sqlH1 = new SqlHandler();
                    result = sqlH1.ExecuteNonQueryTM("Usp_addDebitNoteDetails", parameter1,Conn,trans);
                }
                trans.Commit();
                return i;
                }
        ...
        internal static DataTable getCreditDebitNote(DateTime fromDate, DateTime toDate, long financialYearId)
        {
            try
            {
                try
                {
                    ...
                    parameter.Add(new KeyValuePair<string, object>("@financialYearID", financialYearId));
                    SqlHandler sqlH = new SqlHandler();
                    DataTable dtCreditNoteDetails = sqlH.ExecuteAsDataTable("[dbo].[Usp_getDebitNoteList]", parameter);
        ...
        internal static DataTable getCreditDebitNote(long creditNoteId, long financialYearID)
        {
            try
            {
                try
                {
                    ...
                    parameter.Add(new KeyValuePair<string, object>("@financialYearID", financialYearID));
                    SqlHandler sqlH = new SqlHandler();
                    DataTable dtCreditNoteDetails = sqlH.ExecuteAsDataTable("[dbo].[Usp_getDebitNoteDetails]", parameter);
                    return dtCreditNoteDetails;
                }
                catch (Exception ae)
                {
