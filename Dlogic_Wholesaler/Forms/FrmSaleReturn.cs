    ...
    public partial class FrmSaleReturn : MetroForm
    {
        ...
        public void Lang()
        {
            try
            {
                ...
                {                  
                    lnklblSaleReturnBook.Text = "Sale Return Book(Alt+B)";
                    lblHerader.Text = "Sale Return Details";                 
                    rbMarathi.Text = "Marathi Bill";
                    rbEnglish.Text = "English Bill";
                    metroLabel8.Text = "Bill No:";
                    metroLabel6.Text = "Mobile No:";
                    ...
                    label7.Text = "Code:";
                    label6.Text = "Sale Bill No:";
                    label5.Text = "Sale Date:";
        ...
        private void btnnew_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                txtInvoiceNo.Text = salesReturnDetailsController.getMaxIdSaleRetrunInvoiceId(Utility.FinancilaYearId).ToString();
                dgvItemName.Hide();
        ...
        void tb_TextChanged(Object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                        ...
                        || dgvSaleItems.CurrentCell.ColumnIndex == 17)
                    {
                            ...
                            && dgvSaleItems.CurrentRow.Cells["CGST"].Value != null && dgvSaleItems.CurrentRow.Cells["discount"].Value != null)
                        {
                            ...
                            var rate = (Utility.ParseValue(dgvSaleItems.CurrentRow.Cells["Rate"].Value.ToString()) * 100) / (100 + (Utility.ParseValue(dgvSaleItems.CurrentRow.Cells["IGST"].Value.ToString())));
                            ...
                            dgvSaleItems.CurrentRow.Cells["totalAmount"].Value = Math.Round(Utility.ParseValue(dgvSaleItems.CurrentRow.Cells["Quantity"].Value.ToString()) * rate, 2);
                            dgvItemName.Hide();
                            double purRate = (Utility.ParseValue(dgvSaleItems.CurrentRow.Cells["Quantity"].Value.ToString()) * rate);
                            ...
                            dgvSaleItems.CurrentRow.Cells["totalAmount"].Value = Math.Round((Convert.ToDouble(Utility.ParseValue(dgvSaleItems.CurrentRow.Cells["Quantity"].Value.ToString()) * Utility.ParseValue(dgvSaleItems.CurrentRow.Cells["Rate"].Value.ToString()) - Utility.ParseValue(dgvSaleItems.CurrentRow.Cells["discount"].Value.ToString()))), 2);
                        }
                        else
                        {
                ...
                if (value != null)
                {
                    ...
                    {
                        dtitem = salesReturnDetailsController.getStockInSaleReturnItemDetails(value, "cash", isWholeSale);
                    }
                    if (rbCredit.Checked == true)
                    {
                        dtitem = salesReturnDetailsController.getStockInSaleReturnItemDetails(value, "credit", isWholeSale);
                    }
        ...
        protected void getDataFronCurentRow(int rowIndex)
        {
            try
            {
                ...
                if (row.Cells != null)
                {
                    ...
                    foreach (DataGridViewRow rows in dgvSaleItems.Rows)
                    {
                        if (rows.Cells["itemId"].Value != null && rows.Cells["companyId"].Value != null && rows.Cells["categoryId"].Value != null && rows.Cells["batchNo"].Value != null && rows.Cells["itemName"].Value != null)
                        {
                            if (rows.Cells["itemId"].Value.ToString().Equals(dt.Rows[0]["itemId"].ToString()) && rows.Cells["companyId"].Value.ToString().Equals(dt.Rows[0]["companyId"].ToString()) && rows.Cells["categoryId"].Value.ToString().Equals(dt.Rows[0]["categoryId"].ToString()) && rows.Cells["batchNo"].Value.ToString().Equals(dt.Rows[0]["batchNo"].ToString()) && rows.Cells["itemName"].Value.ToString().Equals(dt.Rows[0]["itemName"].ToString()))
                            {
                                ...
                                if (Utility.Langn == "English")
                                {
                                    MessageBox.Show("This Item Details are available already in list !", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                }
                                else
                                {
                    ...
                    if (rbCash.Checked == true)
                    {
                        rate = Utility.ParseValue(dt.Rows[0]["salesCashRate"].ToString());
                        dgvSaleItems.CurrentRow.Cells["Rate"].Value = Math.Round(rate + (Utility.ParseValue(dt.Rows[0]["IGST"].ToString()) * rate / 100), 2);
                    }
                    if (rbCredit.Checked == true)
                    {
                         rate  = Utility.ParseValue(dt.Rows[0]["salesCreditRate"].ToString());
                        dgvSaleItems.CurrentRow.Cells["Rate"].Value=Math.Round(rate+(Utility.ParseValue(dt.Rows[0]["IGST"].ToString()) * rate / 100), 2);
                    ...
                    double purRate = (Convert.ToDouble(Utility.ParseValue(dgvSaleItems.CurrentRow.Cells["Quantity"].Value.ToString()) * rate));
                    ...
                    dgvSaleItems.CurrentRow.Cells["totalAmount"].Value = Math.Round(Convert.ToDouble(Utility.ParseValue(dgvSaleItems.CurrentRow.Cells["Quantity"].Value.ToString()) * Utility.ParseValue(dgvSaleItems.CurrentRow.Cells["Rate"].Value.ToString()) - Utility.ParseValue(dgvSaleItems.CurrentRow.Cells["discount"].Value.ToString()))).ToString();
                    subtot();
                    //  dgvSaleItems.Focus();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {                  
                    if (Utility.Langn == "English")
                    {
                        MessageBox.Show("Please Fill Or Select Customer Name ..");                     
                    }
                    else
                    {
                ...
                for (int i = 0; i < dgvSaleItems.Rows.Count; i++)
                {
                    ...
                    if (dgvSaleItems.Rows[i].Cells[6].Value == null)
                    {
                        ...
                        if (Utility.Langn == "English")
                        {
                            MessageBox.Show("Please Add BatchNo in list");
                        }
                        else
                        {
                ...
                if (cmbPaymentType.Text.Equals("Cheque"))
                {
                    ...
                    {                       
                        if (Utility.Langn == "English")
                        {
                            MessageBox.Show("Please Add Cheque / DD No..!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                        else
                        {
                ...
                {                    
                    if (Utility.Langn == "English")
                    {
                        MessageBox.Show("Please add Sale Bill No ..");
                    }
                    else
                    {
                ...
                if (isDelete == false)
                {
                    purch.salesReturnInvoiceId = salesReturnDetailsController.getMaxIdSaleRetrunInvoiceId(Utility.FinancilaYearId).ToString();
                }
                else
                {
                ...
                else
                {
                    ...
                    int ks = salesReturnDetailsController.addSalesRetrunDetails(purch, purchase);
                if (ks > 0)
                {
        ...
        private void FrmSaleReturn_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                {
                    FrmTempSaleReturnTemp frm = new FrmTempSaleReturnTemp();
                    frm.ShowDialog();
                }
            }
            catch (Exception ex)
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable dtSaleInvoice = salesReturnDetailsController.getSaleReturnIvoiceList(Convert.ToDateTime(dtpFromChallanDate.Value.ToShortDateString()), Convert.ToDateTime(dtpToChallanDate.Value.ToShortDateString()),Utility.FinancilaYearId);
        ...
        public void BindData(long salesInvoiceId)
        {
            try
            {
                ...
                isDelete = true;
                IGST5 = 0; IGST12 = 0; IGST18 = 0; IGST28 = 0;
                DataTable dtSaleChallen = saleReportController.getsaleReturnDetails(salesInvoiceId, Utility.FinancilaYearId);
                ...
                foreach (DataRow dr in dtSaleChallen.Rows)
                {
                    ...
                    var rate = Utility.ParseValue(dr["perQtysalePrice"].ToString());
                    dgvSaleItems.CurrentRow.Cells["Rate"].Value = Math.Round(rate + (Utility.ParseValue(dr["IGST"].ToString()) * rate / 100), 2);
                    dgvSaleItems.CurrentRow.Cells["discount"].Value = 0;
                    ...
                    double purRate = Math.Round(Convert.ToDouble(Utility.ParseValue(dgvSaleItems.CurrentRow.Cells["Quantity"].Value.ToString()) * rate),2);
