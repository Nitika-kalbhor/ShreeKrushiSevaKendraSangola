...
{
    public partial class frmSaleInvoive : MetroForm
    {
        public frmSaleInvoive()
        {
            InitializeComponent();
        }
        ...
        public void Lang()
        {
            try
            {
                if (Utility.Langn == "English")
                {
                    ...
                    dgvSaleInvoice.Columns[2].HeaderText = "Customer Name";
                    dgvSaleInvoice.Columns[3].HeaderText = "Date";
                    dgvSaleInvoice.Columns[4].HeaderText = "Amount";
        ...
        private void btnnew_Click(object sender, EventArgs e)
        {
            ...
            {               
                ...
                {
                    txtInvoiceNo.Text = SaleDetailsController.getWholeMaxIdSaleInvoiceId(Utility.FinancilaYearId).ToString();
                    isPesticideId = SaleDetailsController.getWholeMaxIdSalesItemDetails(1, Utility.FinancilaYearId).ToString();
                    isFertilizerId = SaleDetailsController.getWholeMaxIdSalesItemDetails(2, Utility.FinancilaYearId).ToString();
                    isSeedsId = SaleDetailsController.getWholeMaxIdSalesItemDetails(3, Utility.FinancilaYearId).ToString();
                    isPGROther = SaleDetailsController.getWholeMaxIdSalesItemDetails(4, Utility.FinancilaYearId).ToString();
                }
                else
                {
                    txtInvoiceNo.Text = SaleDetailsController.getMaxIdSaleInvoiceId(Utility.FinancilaYearId).ToString();
                    isPesticideId = SaleDetailsController.getMaxIdSalesItemDetails(1, Utility.FinancilaYearId).ToString();
                    isFertilizerId = SaleDetailsController.getMaxIdSalesItemDetails(2, Utility.FinancilaYearId).ToString();
                    isSeedsId = SaleDetailsController.getMaxIdSalesItemDetails(3, Utility.FinancilaYearId).ToString();
                    isPGROther = SaleDetailsController.getMaxIdSalesItemDetails(4, Utility.FinancilaYearId).ToString();
                }
                        ...
                        SaleDetailsController.deleteSalesInvoiceRow(saleUpdateId, txtStateCode.Text.Trim(), Convert.ToInt64(dgvSaleItems.Rows[rowIndex].Cells["salesItemDetailsId"].Value)
                            ...
                            Convert.ToString(dgvSaleItems.Rows[rowIndex].Cells["batchNo"].Value),
                                                                     Utility.ParseValue(dgvSaleItems.Rows[rowIndex].Cells["Quantity"].Value.ToString())
                                                                      , Convert.ToInt64(dgvSaleItems.Rows[rowIndex].Cells["StockId"].Value), Utility.ParseValue(dgvSaleItems.Rows[rowIndex].Cells["IGSTAmt"].Value.ToString()), Utility.ParseValue(dgvSaleItems.Rows[rowIndex].Cells["IGST"].Value.ToString()), chkholeSale.Checked);
                    }
                }
        ...
        protected void getDataFronCurentRow(int rowIndex)
        {
            try
            {
                ...
                if (row.Cells != null)
                {
                    ...
                    if (Convert.ToDouble(dt.Rows[0]["currentStock"]) <= 0)
                    {
                        ...
                        if (dr1 == DialogResult.Yes)
                        {
                            ...
                            if (isExpire == true)
                            {
                                ...
                                if (rbtnCash.Checked == true && chkholeSale.Checked == false)
                                {
                                    dgvSaleItems.CurrentRow.Cells["Rate"].Value = Utility.ParseValue(dt.Rows[0]["salesCashRate"].ToString());
                                }
                                if (rbtnCredit.Checked == true && chkholeSale.Checked == false)
                                {
                                    dgvSaleItems.CurrentRow.Cells["Rate"].Value = Utility.ParseValue(dt.Rows[0]["salesCreditRate"].ToString());
                                }
                                if (rbtnCash.Checked == true && chkholeSale.Checked == true)
                                {
                                    dgvSaleItems.CurrentRow.Cells["Rate"].Value = Utility.ParseValue(dt.Rows[0]["wholeSalesCashRate"].ToString());
                                }
                                if (rbtnCredit.Checked == true && chkholeSale.Checked == true)
                                {
                                    dgvSaleItems.CurrentRow.Cells["Rate"].Value = Utility.ParseValue(dt.Rows[0]["wholeSalesCreditRate"].ToString());
                                }
                    ...
                    {
                        if (Convert.ToDateTime(dt.Rows[0]["expireDate"]) <= DateTime.Now && Convert.ToBoolean(dt.Rows[0]["isExpiryApplicable"]) == true)
                        {
                            DialogResult showReport = MessageBox.Show(/*Convert.ToString(dt.Rows[0]["itemName"]) + Convert.ToString(dt.Rows[0]["expireDate"]) + */" वस्तु एक्सपायर झाली आहे, विकायची आहे का ?", "", MessageBoxButtons.YesNo);
                            if (showReport == DialogResult.Yes)
                            {
        ...
        private void savebill(object sender, EventArgs e)
        {
            try
            {
                ...
                //if (txtNewSaleInvoiceNo.Text == string.Empty)
                //{
                //    if (Utility.Langn == "English")
                //    {
                ...
                if (isupdate == false)
                {
                    ...
                    {
                        purch.salesInvoiceId = SaleDetailsController.getMaxIdSaleInvoiceId(Utility.FinancilaYearId).ToString();
                    }
                    else
                    {
                        purch.salesInvoiceId = SaleDetailsController.getWholeMaxIdSaleInvoiceId(Utility.FinancilaYearId).ToString();
                    }                   
                }
                else
                {
                ...
                purch.NewsalesInvoiceId = purch.salesInvoiceId;
                ...
                if (chkholeSale.Checked == true)
                {
                    purch.isWholeSale = true;
                }
                else
                {
                    purch.isWholeSale = false;
                }
                ...
                purch.driverName = txtDriverName.Text;
                purch.gadiNo = txtGadino.Text;
                ...
                {
                    ks = SaleDetailsController.addSalesDetailsForConvertChalan(purch, purchase);
                    if (ks > 0)
                    {
                ...
                else
                {
                    if (isDelete == true)
                    {
                        ...
                        {
                            purch.isDelete = true;
                            ks = SaleDetailsController.addSalesDetails(purch, purchase);
                        }
                        ...
                        if (ks > 0)
                        {
                            //   MsgBox.Show("सदर चलन यशस्वीरित्या डिलीट केले गेले आहे ..!", "Save", MsgBox.Buttons.OK);
        ...
        public void SendSMS()
        {
            ...
            //{
            //    if (lblCreditAmount.Text == string.Empty)
            //    {
            //        lblCreditAmount.Text = "0";
            //    }
            //    string sms = @"MahaShakti Krushi Kendra, Laxmi Takali Date-" + dtpChallenDate.Value.ToString("dd/MM/yyyy") + " Bill No- " + txtInvoiceNo.Text
            //        + " Amount - " + txtTotalAmount.Text +" Total Due-" + ((Convert.ToDouble(lblCreditAmount.Text) + Convert.ToDouble(txtTotalAmount.Text)) - Convert.ToDouble(txtPaidAmount.Text)).ToString();

            //    //    + " From-Shriraj Krushi Kendra Akluj.";
            //  SMSUtility.SMSSendFEESINSTALLMENT(sms, txtmobile.Text.Trim());
            //}
        }
        ...
        private void frmSaleInvoive_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                frmTempSaleInvoice frm = new frmTempSaleInvoice();
                frm.ShowDialog();
            }
          ...
          try
            {
                ...
                //    if (txtNewSaleInvoiceNo.Text == string.Empty)
                //    {
                //        MessageBox.Show("कृपया सेल बिल नं भरा.");
                //        txtNewSaleInvoiceNo.Focus();
                //        return;
                //    }
                //    else
                ...
                //        {
                //            Boolean isExists = SaleDetailsController.getInvoice(txtNewSaleInvoiceNo.Text, Convert.ToInt64(cmbCustomerName.SelectedValue), Utility.FinancilaYearId);
                //            if (isExists == true)
                //            {
                //                MessageBox.Show("सदर सेल बिल भरले गेले आहे.");
                //                dgvSaleItems.Enabled = false;
                //                txtNewSaleInvoiceNo.Focus();
                //            }
                //            else
        ...
        public void bill()
        {
            ...
            {
                SaleID = txtInvoiceNo.Text;
                DataTable dtSale = ReportController.getSaleInvoice(SaleID, Utility.FinancilaYearId);
                RptCustomerBill rpt = new RptCustomerBill();
                RptSaleBill frm = new RptSaleBill();
                rpt.SetDataSource(dtSale);
                frm.crystalReportViewer1.ReportSource = rpt;
        ...
        public void BindData(long salesInvoiceId)
        {
            try
            {
                ...
                chkholeSale.Enabled = false;
                if (chkholeSale.Checked == true)
                {
                    dtSaleChallen = SaleDetailsController.getWholeSaleInvoice(salesInvoiceId.ToString(), Utility.FinancilaYearId);
                }
                else
                {
                    dtSaleChallen = SaleDetailsController.getSaleInvoice(salesInvoiceId.ToString(), Utility.FinancilaYearId);
                }
                saleUpdateId = salesInvoiceId.ToString();
                txtInvoiceNo.Text = dtSaleChallen.Rows[0]["salesInvoiceId"].ToString();
               // txtNewSaleInvoiceNo.Text = dtSaleChallen.Rows[0]["NewsalesInvoiceId"].ToString();
                ...
                txtDriverName.Text = dtSaleChallen.Rows[0]["driverName"].ToString();
                txtGadino.Text = dtSaleChallen.Rows[0]["gadiNo"].ToString();
        ...
        private void btnSaleInvoiceList_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (chkholeSale.Checked == true)
                {
                    saleDetails = SaleDetailsController.getWholeSaleIvoiceList(Convert.ToDateTime(dtpFromChallanDate.Value.ToShortDateString()), Convert.ToDateTime(dtpToChallanDate.Value.ToShortDateString()));
                }
                else
                {
                    saleDetails = SaleDetailsController.getSaleIvoiceList(Convert.ToDateTime(dtpFromChallanDate.Value.ToShortDateString()), Convert.ToDateTime(dtpToChallanDate.Value.ToShortDateString()));
                }
                dgvSaleInvoice.DataSource = saleDetails;
        ...
        private void btnShow_Click(object sender, EventArgs e)
        {
            try
            {
                DataTable dtSaleInvoice = SaleDetailsController.getSaleIvoiceList(Convert.ToDateTime(dtpFromChallanDate.Value.ToShortDateString()), Convert.ToDateTime(dtpToChallanDate.Value.ToShortDateString()));
        ...
        private void btnConvertChallan_Click(object sender, EventArgs e)
        {
            try
            {
                frmSaleChallanList frm = new frmSaleChallanList();
                frm.IdentityUpdated += new frmSaleChallanList.IdentityUpdateHandler(IdForm_ButtonClicked);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void IdForm_ButtonClicked(object sender, frmSaleChallanList.IdentityUpdateEventArgs e)
        {
            try
            {
                btnnew_Click(sender, e);

                DataTable dtpurchaseChallan = SaleDetailsController.getSaleChallenDetails(0, Utility.FinancilaYearId);

                foreach (DataRow dr in e.dt.Rows)
                {
                    challanNo += "," + dr[0].ToString();
                    DataTable dts = SaleDetailsController.getSaleChallenDetails(Convert.ToInt64(dr["SalesChallanBillId"]), Utility.FinancilaYearId);
        ...
        private void dgvminimumperchase_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {

        }

        private void txtNewSaleInvoiceNo_Leave(object sender, EventArgs e)
        {           
            try
            {
                if (cmbCustomerName.SelectedIndex > 0 && txtInvoiceNo.Text!=string.Empty)
                {
                         if (isupdate== false && isDelete==false)
                         {
                             Boolean isExists = SaleDetailsController.getInvoice(txtNewSaleInvoiceNo.Text, Convert.ToInt64(cmbCustomerName.SelectedValue), Utility.FinancilaYearId);
                             ...
                             {                                 
                                 if (Utility.Langn == "English")
                                 {
                                     MessageBox.Show("This Sale Bill already Saved.");
                                 }
                                 else
                                 {
                                     MessageBox.Show("सदर सेल बिल भरले गेले आहे.");
