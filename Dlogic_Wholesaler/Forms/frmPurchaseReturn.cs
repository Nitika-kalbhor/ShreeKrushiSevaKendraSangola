    ...
    public partial class frmPurchaseReturn : MetroForm
    {
        ...
        public void Lang()
        {
            try
            {
                ...
                {
                    lnklblPurchaseReturnBook.Text = "Purchase Return Book(Alt+P)";
                    lblHerader.Text = "Purchase Return Details";
                    metroLabel2.Text = "Bill No:";
                    metroLabel1.Text = "Date:";
                    lblstockInNo.Text = "Dealer Name:";
                    metroLabel3.Text = "Mobile No:";
                    label6.Text = "Purchase Bill No:";
                    label1.Text = "Purchase State:";
                    label9.Text = "Code:";
                    ...
                    button2.Text = "Bill List";
                    btnPrint.Text = "Print";
          
                    dgvPurchaseItems.Columns[5].HeaderText = "Naration";
                    dgvPurchaseItems.Columns[6].HeaderText = "BatchNo";                
                    dgvPurchaseItems.Columns[7].HeaderText = "Qty";
                    dgvPurchaseItems.Columns[8].HeaderText = "freeQuantity";
                    dgvPurchaseItems.Columns[9].HeaderText = "UnitBy";                   
                    dgvPurchaseItems.Columns[10].HeaderText = "Pur.Rate";
        ...
        private void btnnew_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                purchaseReutrnId = 0; 
                BindComboBoxgetDealerName();          
                txtInvoiceNo.Text = purchaseDetailsController.getMaxIdPurchaseReturnInvoiceId(Utility.FinancilaYearId).ToString();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {                   
                    if (Utility.Langn == "English")
                    {
                        MessageBox.Show("Please Add Total Amount..!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    else
                    {
                ...
                if(txtPurchaseBillNo.Text==string.Empty)
                {
                    if (Utility.Langn == "English")
                    {
                        MessageBox.Show("Please Fill Purchase Bill No..!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    else
                    {
                        MessageBox.Show("कृपया खरेदी क्रमांक  भरा ..!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                ...
                if (isDelete == false)
                {
                    purch.purchaseReutrnInvoiceId = purchaseDetailsController.getMaxIdPurchaseReturnInvoiceId(Utility.FinancilaYearId).ToString();
                }
                else
                {
                ...
                int l = purchaseReturnDetailsController.addPurchaseReturnDetails(purch, PurchsSale);
                if (l > 0)
                {
                    if (isDelete == false)
                    {
                        DialogResult report = DialogResult.No;
                        if (Utility.Langn == "English")
                        {
                           report= MessageBox.Show("Do you want to print?", "Print", MessageBoxButtons.YesNo);
                        }
                        else{
                            report = MessageBox.Show("तुम्हाला प्रिंट हवी आहे का?", "प्रिंट", MessageBoxButtons.YesNo);
                        }
                        ...
                        else
                        {
                            if (Utility.Langn == "English")
                            {
                                MessageBox.Show("Current Bill Saved successfully..", "Save", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                            else
                            {
                    ...
                    if(isDelete==true)
                    {
                        ...
                        {
                            MessageBox.Show("सदर माहिती यशस्वीरित्या डिलीट केले गेले आहे ..!", "Delete", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                    }
        ...
        public void billWithGSTSlot()
        {
            try
            {
               string purchaseReutrnInvoiceId = txtInvoiceNo.Text;
                DataTable dtSale = new DataTable();
                dtSale = ReportController.getPurchaseReturn(purchaseReutrnInvoiceId, Utility.FinancilaYearId);
                string amountInWord = "";
               
                    amountInWord = Utility.NumberToWordMarathi(Convert.ToInt64(dtSale.Rows[0]["netBillAmount"].ToString())) + " " + "Rs. /Only.";
        ...
        private void frmPurchaseReturn_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                {
                    frmTempPurchaseReturn frm = new frmTempPurchaseReturn();
                    frm.ShowDialog();
                }
            }
            catch (Exception ex)
        ...
        private void dgvSaleChallan_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                DataTable dtSaleChallen = purchaseReturnDetailsController.getPurchaseReturnIvoiceDetails(purchaseReutrnId,Utility.FinancilaYearId);
        ...
        public void BindData(DataTable dtSaleChallen)
        {
            try
            {
                ...
                dtpPurchaseDate.Text = dtSaleChallen.Rows[0]["purchaseDate"].ToString();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            dgvSaleChallan.DataSource = purchaseReturnDetailsController.getPurchaseReturnList(Convert.ToDateTime(dtpFromChallanDate.Value.ToShortDateString()), Convert.ToDateTime(dtpToChallanDate.Value.ToShortDateString()),Utility.FinancilaYearId);
