...
{
    public partial class frmTempPurchaseChalan : MetroForm
    {
        public frmTempPurchaseChalan()
        {
            InitializeComponent();
        }
            ...
            try
            {
                if (cmbDealerName.SelectedIndex > 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Boolean isExists = TempPurchaseDetailsController.getpurchaseChalanNo(txtInvoiceNo.Text, Convert.ToInt64(cmbDealerName.SelectedValue), Utility.FinancilaYearId);
                            if (isExists == true)
                            {
        ...
        private void btnnew_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                BindComboBoxgetDealerName();
                purchaseChalanId = TempPurchaseDetailsController.getMaxIdPurchaseChalanId();
                dgvItemName.Hide();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                   purch.purchaseChalanId=TempPurchaseDetailsController.getMaxIdPurchaseChalanId();
                }
                ...
                if (isDelete == true)
                {
                    DialogResult dr = MessageBox.Show("तुम्हाला सदर चलन Delete करायाचे आहे काय?", "Confirmation", MessageBoxButtons.YesNo);
                    if (dr == DialogResult.Yes)
                    {
                        purch.isDelete = true;
                        ks = TempPurchaseDetailsController.addPurchaseChalanDetails(purch);
                    }
                    ...
                    if (ks > 0)
                    {
                        MessageBox.Show("रेकॉर्ड डिलीट केला गेला !", "माहिती", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    else if (isDelete == false)
                    {
                        ...
                        if (ks > 0)
                        {
                            MessageBox.Show("सदर चलन यशस्वीरित्या साठवले गेले आहे..!", "जतन", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void btnChallanList_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dtpFromChallanDate.Value = Utility.firstDate;
                dtpToChallanDate.Value = Utility.lastDate;
                dgvSaleChallan.DataSource = TempPurchaseDetailsController.getChallenList(Convert.ToDateTime(dtpFromChallanDate.Value.ToShortDateString()),Convert.ToDateTime(dtpToChallanDate.Value.ToShortDateString()),Utility.FinancilaYearId);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        private void btnShow_Click(object sender, EventArgs e)
        {
                try
            {
                DataTable dtChallaneList = TempPurchaseDetailsController.getChallenList(Convert.ToDateTime(dtpFromChallanDate.Value.ToShortDateString()), Convert.ToDateTime(dtpToChallanDate.Value.ToShortDateString()), Utility.FinancilaYearId);
        ...
        private void deleteRowToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                if (isupDate == true)
                {
                    ...
                    {
                        TempPurchaseDetailsController.deleteRow(purchaseChalanId, Convert.ToInt64(dgvPurchaseItems.Rows[rowIndex].Cells["purchaseChalanItemDetailId"].Value),
          ...
          public void BindData(long SalesChallanBillId)
        {
            try
            {
                ...
                isDelete =false;
                isupDate = true;
                DataTable dtSaleChallen = TempPurchaseDetailsController.getSaleChallenDetails(purchaseChalanId,Utility.FinancilaYearId);
